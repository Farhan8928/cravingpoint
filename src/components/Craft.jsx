 ...
 *
 * Where the hero holds one piece of copy and fades it out, this hands off
 * between three captions as the wrap is built, each crossfading over its own
 * third of the scroll. The captions are why this is not just "the hero again
 * with different footage": the film is a process, so the type narrates it.
 *
 * Only three captions for 151 frames. The temptation is one per beat, but each
 * extra caption shortens the time the reader gets with the frame, and at four
 * they swap faster than they can be read.
 *
 * Type here uses the fixed `film-*` colours — the footage is dark in both
 * themes, so theme-following text would vanish in light mode.
 */
const BEATS = [
  {
    label: 'On the coals',
  ...
  useEffect(() => {
    ...
    beats.forEach((beat, i) => {
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: section,
          // Each caption owns its slice, with a lead-in and lead-out so two are
          // never fully opaque at once.
      ...
      tl.fromTo(beat, { opacity: 0, y: 36 }, { opacity: 1, y: 0, duration: 1, ease: 'power2.out' })
        .to(beat, { opacity: 1, duration: 2 })
        .to(beat, { opacity: 0, y: -36, duration: 1, ease: 'power2.in' });
  ...
  return (
    ...
    >
      {/* The subject of this film sits low and centre-right, directly under the
          captions. FrameSequence's own scrim is symmetrical and does nothing for
          type over the tortilla, so this section adds a directional one:
          left-weighted on desktop where the captions are, bottom-weighted on
          mobile where they stack. */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_top,#100c09_0%,rgba(16,12,9,0.82)_34%,transparent_68%)] md:bg-[linear-gradient(to_right,#100c09_0%,rgba(16,12,9,0.88)_28%,rgba(16,12,9,0.6)_50%,transparent_78%)]"
      />
      ...
      <div ref={wrapRef} className="absolute inset-0 mx-auto max-w-container px-gutter">
        {/* This inner box carries no padding of its own, so the absolutely
            positioned captions resolve against a box that already respects the
            gutter — `inset-x-0` on a padded ancestor would sit flush to the
        ...
        <div className={`relative h-full ${reduced ? 'flex flex-col justify-center gap-8' : ''}`}>
          {BEATS.map((beat) => (
            <div
              ...
              // visible one to the bottom on mobile, where the scrim is heaviest
              // and the frame is emptiest.
              className={
                reduced
                  ? ''
            ...
            >
              ...
              <p className="mt-4 max-w-prose text-body-lg text-film-muted">{beat.body}</p>
            </div>
          ))}
        </div>
      </div>
